...
void OutputHeadOfCollumnsByAllString(ColumnName * pHeadAr, int nSize);
...
void DeleteOneStudents(Students * pStAr, int nSize, int nNumber);
...
void SortingStudentsByNumber(Students * pArr, int nSize, bool(*pFn)(int, int));
void SortingStudentsByAge(Students * pArr, int nSize, bool(*pFn)(int, int));
void SortingStudentsByGrade(Students * pArr, int nSize, bool(*pFn)(int, int));
bool CheckIfCompareForAsc(int a, int b);
bool CheckIfCompareForDesc(int a, int b);
...
int main()
{
	...
	do
	{
		cout << endl << "For edit the student please press E, for delete student D or quit Q: ";
		c = _getch();
		system("cls");
	} while (c != 101 && c != 100 && c != 113);
	...
	case 100: // Delete the student
	{
		...
		system("cls");
		cout << "The student wich you choice to delete: " << endl;
		OutputHeadOfCollumns(pHeadOfCollumns, CollumnsCount);
		OutputOneStudents(pStudentsArray, StudentNumber - 1);

		cout << "Confirm deleting by press Y-yes or N-no: ";
		int e;
		do
		{
			e = _getch();
		} while (e != 121 && e != 110); // Y - 121, N - 110

		system("cls");

		if (e == 121)
		{
			//DeleteOneStudents(pStudentsArray, StudentsCount, StudentNumber - 1);

			int Size = StudentsCount - 1;
			Students * pStTmp = new Students[Size];
			...
			for (int i = 0; i < Size; ++i)
			{
				if (i == StudentNumber - 1)
					++ind;
				j = i + ind;
				pStTmp[i].Student_Number = pStudentsArray[j].Student_Number;
				memcpy(pStTmp[i].First_Name, pStudentsArray[j].First_Name, sizeof(pStudentsArray[j].First_Name));
				memcpy(pStTmp[i].Second_Name, pStudentsArray[j].Second_Name, sizeof(pStudentsArray[j].Second_Name));
				pStTmp[i].Student_Age = pStudentsArray[j].Student_Age;
				for (int k = 0; k < RATING_COUNT; ++k)
					pStTmp[i].Student_Ratings[k] = pStudentsArray[j].Student_Ratings[k];
				pStTmp[i].Student_Grade = pStudentsArray[j].Student_Grade;
			}
			delete[] pStudentsArray;
			pStudentsArray = pStTmp;
	...
	cout << endl << "Choice by press the number of what you will change: " << endl;
	OutputHeadOfCollumnsByAllString(pHeadOfCollumns, CollumnsCount);

	int f;
	do
	{
		cout << "What row do you want to sort (enter the number of row): ";

		f = _getch();
		system("cls");
	} while (f != 49 && f != 52 && f != 54); // 1, 4, 6
	...
	case 49: // 1
	{
		...
		do
		{
			cout << "Enter a type of Sort (A - for Ascending; D - for Descending) or Q for quit: ";

			g = _getch();
			system("cls");
		} while (g != 97 && g != 100 && g != 113); // a, d, q

		if (g == 97)
			SortingStudentsByNumber(pStudentsArray, StudentsCount, CheckIfCompareForAsc);
		else if (g == 100)
			SortingStudentsByNumber(pStudentsArray, StudentsCount, CheckIfCompareForDesc);
		else
			break;
	...
	case 52: // 4
	{
		...
		if (g == 97)
			SortingStudentsByAge(pStudentsArray, StudentsCount, CheckIfCompareForAsc);
		else if (g == 100)
			SortingStudentsByAge(pStudentsArray, StudentsCount, CheckIfCompareForDesc);
		else
			break;
	...
	case 54: // 1
	{
		...
		if (g == 97)
			SortingStudentsByGrade(pStudentsArray, StudentsCount, CheckIfCompareForAsc);
		else if (g == 100)
			SortingStudentsByGrade(pStudentsArray, StudentsCount, CheckIfCompareForDesc);
		else
			break;
...
void DeleteOneStudents(Students * pStAr, int nSize, int nNumber)
{
	...
	for (int i = 0; i < Size; ++i)
	{
		if (i == nNumber)
			++ind;
		j = i + ind;
		pStTmp[i].Student_Number = pStAr[j].Student_Number;
		memcpy(pStTmp[i].First_Name, pStAr[j].First_Name, sizeof(pStAr[j].First_Name));
		memcpy(pStTmp[i].Second_Name, pStAr[j].Second_Name, sizeof(pStAr[j].Second_Name));
		pStTmp[i].Student_Age = pStAr[j].Student_Age;
		for (int k = 0; k < RATING_COUNT; ++k)
			pStTmp[i].Student_Ratings[k] = pStAr[j].Student_Ratings[k];
		pStTmp[i].Student_Grade = pStAr[j].Student_Grade;
	}
	delete[] pStAr;
	pStAr = pStTmp;
...
void SortingStudentsByNumber(Students * pArr, int nSize, bool(*pFn)(int, int))
{
	...
	for (int i = 0; i < nSize; ++i)
	{
		...
		for (int j = i + 1; j < nSize; ++j)
			if (pFn(pArr[j].Student_Number, nMinSt.Student_Number))
			{
				nMinSt = pArr[j];
				iMintInd = j;
			}
...
void SortingStudentsByAge(Students * pArr, int nSize, bool(*pFn)(int, int))
{
	...
	for (int i = 0; i < nSize; ++i)
	{
		...
		iMintInd = i;
		for (int j = i + 1; j < nSize; ++j)
			if (pFn(pArr[j].Student_Age, nMinSt.Student_Age))
			{
				nMinSt = pArr[j];
				iMintInd = j;
			}
...
void SortingStudentsByGrade(Students * pArr, int nSize, bool(*pFn)(int, int))
{
	...
	for (int i = 0; i < nSize; ++i)
	{
		...
		iMintInd = i;
		for (int j = i + 1; j < nSize; ++j)
			if (pFn(pArr[j].Student_Grade, nMinSt.Student_Grade))
			{
				nMinSt = pArr[j];
				iMintInd = j;
			}
